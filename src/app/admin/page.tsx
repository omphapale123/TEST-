...
import { DollarSign, Repeat, Users, CheckSquare, BarChart3, Users2, Loader2, Briefcase } from 'lucide-react';
...
function AdminDashboardContent() {
  ...
  const { data: chats, isLoading: isLoadingChats } = useCollection(chatsQuery);
  ...
  const engagedBuyers = useMemo(() => {
    if (!chats) return 0;
    const buyerIds = new Set(chats.map(c => c.buyerId).filter(Boolean));
    return buyerIds.size;
  }, [chats]);

  const engagedSuppliers = useMemo(() => {
    if (!chats) return 0;
    const supplierIds = new Set(chats.map(c => c.supplierId).filter(Boolean));
    return supplierIds.size;
  }, [chats]);

  const isLoading = isLoadingUsers || isLoadingPending || isLoadingTrades || isLoadingChats;
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">From finished trades</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Finished trades</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Engaged Buyers</CardTitle>
              <Users className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              {isLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : <div className="text-2xl font-bold">{engagedBuyers}</div>}
              <p className="text-xs text-muted-foreground">Active in chats</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : <div className="text-2xl font-bold">{engagedSuppliers}</div>}
              <p className="text-xs text-muted-foreground">Active in chats</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Pending Verif.</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                <Link href="/admin/verify-documents" className="hover:underline text-primary">Review</Link>
              </p>
            </CardContent>
          </Card>
          <Card>
